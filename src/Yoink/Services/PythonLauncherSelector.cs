...
internal static partial class PythonLauncherSelector
{
    ...
    public static string BuildOnnxRuntimeMissingVersionMessage(IEnumerable<LauncherEntry> entries)
    {
        ...
        return discovered.Count == 0
            ? "Yoink needs Python 3.10, 3.11, or 3.12 installed to set up the local sticker and upscale runtimes."
            : $"Yoink needs Python 3.10, 3.11, or 3.12 to set up the local sticker and upscale runtimes. Found: {string.Join(", ", discovered)}.";
    }
