...
- Files changed: `src/OddSnap/Services/HotkeyService.cs`
- How I validated it: `lsp_diagnostics` reported zero errors for `HotkeyService.cs`. Standard `dotnet build`/`dotnet test` validation is currently blocked by existing workspace build artifacts and a running `OddSnap.exe` locking the default debug output.
...
- Files changed: `src/OddSnap/Services/OcrService.cs`
...
- Files changed: `src/OddSnap/Capture/DxgiScreenCapture.cs`
...
- Files changed: `src/OddSnap/Capture/VideoRecorder.cs`
...
- Files changed: `src/OddSnap/Services/ClipboardService.cs`
...
- Files changed: `src/OddSnap/Services/PythonLauncherSelector.cs`, `src/OddSnap/Services/RembgRuntimeService.cs`, `src/OddSnap/Services/UpscaleRuntimeService.cs`, `src/OddSnap/UI/OcrResultWindow.xaml.cs`, `src/OddSnap/UI/ToolListBuilder.cs`, `tests/OddSnap.Tests/PythonLauncherSelectorTests.cs`
- How I validated it: `dotnet test tests\OddSnap.Tests\OddSnap.Tests.csproj --filter PythonLauncherSelectorTests` passed (`9/9`). A full `dotnet build`/`dotnet test` run was partially blocked by a separate running `dotnet.exe` locking the default WPF build output in the workspace.
...
- Files changed: `src/OddSnap/UI/PreviewWindow.Actions.cs`
...
- Why it was a problem: The app was paying a large baseline memory cost even while mostly idle. Startup thumbs/search warming and resident embedding arrays made OddSnap look leakier than it was, especially with large screenshot histories.
- Files changed: `src/OddSnap/App/App.Startup.cs`, `src/OddSnap/App/App.Lifecycle.cs`, `src/OddSnap/Services/LocalClipRuntimeService.cs`, `src/OddSnap/Services/ImageSearchIndexService.cs`, `src/OddSnap/Services/ImageSearchIndexService.Indexing.cs`, `src/OddSnap/Services/ImageSearchIndexService.Search.cs`, `src/OddSnap/UI/Settings/Infrastructure/SettingsMediaCache.cs`
- How I validated it: `dotnet test tests\OddSnap.Tests\OddSnap.Tests.csproj --filter "PythonLauncherSelectorTests|ImageSearchQueryMatcherTests"` passed (`16/16`). File-level diagnostics for the edited sources reported zero diagnostics. Full-solution WPF rebuilds remain noisy because of existing workspace-generated `obj`/`wpftmp` state unrelated to these edits.
...
- Files changed: `src/OddSnap/Services/SoundService.cs`
- How I validated it: File-level diagnostics reported zero diagnostics, and `dotnet test tests\OddSnap.Tests\OddSnap.Tests.csproj --filter "PythonLauncherSelectorTests|ImageSearchQueryMatcherTests|AppSettingsTests"` passed (`25/25`).
