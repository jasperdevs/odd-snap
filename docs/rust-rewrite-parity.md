...
- Rust app now has a platform-neutral annotation workspace foundation for committed annotations, previews, selection, move/scale operations, and undo/redo before the production GPUI drawing surface is wired.
...
- Imported media history preserves practical legacy media metadata including dimensions, file sizes, upload state, captured time, and reusable managed video thumbnails.
...
- macOS sticker/upscale routes use the native interactive `screencapture -i` selection foundation and then run the shared Rust processing flow, instead of being hard-blocked on the future production overlay.
...
- GPUI lifecycle controls can open the official OddSnap releases page as the safe update fallback while automatic install/apply remains pending.
...
- GPUI media history bulk copy, upload, selection, and removal actions target the currently visible row window so limited/search-filtered large histories do not mutate hidden rows.
...
- GPUI can open an upscale result preview window with before/after images, split/before/after preview modes, and copy/open actions after a processed capture when the migrated preview-window preference is enabled.
...
- The GPUI shell exposes translation model/source/target controls backed by the shared Rust language normalization rules, so migrated translation settings are not read-only.
...
Sticker/upscale parity still depends on production overlay polish and real-device validation; the Rust local rembg/ONNX runtime foundation, macOS native selection route, basic GPUI provider/model/API-key controls, and interactive upscale preview modes are present, but real-device runtime smoke verification still needs to land before full feature parity can be claimed.
